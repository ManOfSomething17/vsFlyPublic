...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	function reloadSelectedCharacter() {
		...
		if(characterFile.confirm_offset == null) characterFile.confirm_offset = [0, 0]; // crash prevention i hope
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput) {
			...
			{
				if(FlxG.keys.justPressed.LEFT) {
					characterFile.confirm_offset[0] += shiftMult;
					updateOffsetText();
				}
				if(FlxG.keys.justPressed.RIGHT) {
					characterFile.confirm_offset[0] -= shiftMult;
				...
				if(FlxG.keys.justPressed.UP) {
					characterFile.confirm_offset[1] += shiftMult;
					updateOffsetText();
				}
				if(FlxG.keys.justPressed.DOWN) {
					characterFile.confirm_offset[1] -= shiftMult;
			...
			if(FlxG.keys.pressed.SPACE && curTypeSelected == 1) {
				grpWeekCharacters.members[curTypeSelected].animation.play('confirm', true);
				playingConfirm = true;
			}

			if(playingConfirm) grpWeekCharacters.members[curTypeSelected].offset.set(characterFile.confirm_offset[0], characterFile.confirm_offset[1]);
		}
	...
	function updateOffsetText() {
		txtOffsets.text = 'Idle Offset: ${characterFile.position}' + ((curTypeSelected == 1) ? ' / Confirm Offset: ${characterFile.confirm_offset}' : '');
	}
