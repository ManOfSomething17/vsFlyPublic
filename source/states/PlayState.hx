...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['i hate my life', 0.2], //From 0% to 19%
		['Oh Boy!', 0.4], //From 20% to 39%
	...
	public var metadata:SongMeta;
	public var hasMeta:Bool;
	public var songPopup:SongCredit;
	...
	override public function create()
	{
		...
		var metaFile = Paths.json(songName + '/meta');

		#if MODS_ALLOWED
		if (!FileSystem.exists(metaFile))
			metaFile = Paths.modsJson(songName + '/meta');
		#end

		if (FileSystem.exists(metaFile))
		{
			metadata = cast Json.parse(File.getContent(metaFile));
			hasMeta = true;
		}
		else
		{
			trace('$songName has no metadata');
			hasMeta = false;
		}
		...
		stageOverlay = new FlxSprite(FlxG.width * -0.5, FlxG.height * -0.5).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		stageOverlay.scrollFactor.set();
		stageOverlay.alpha = ClientPrefs.data.stageDimming;
		if(ClientPrefs.data.stageDimming != 0) add(stageOverlay);
		...
		songPopup = new SongCredit((hasMeta && metadata.title != null) ? metadata.title : songName, (hasMeta && metadata.artist != null) ? metadata.artist : null);
		songPopup.scrollFactor.set();
		uiGroup.add(songPopup);
		...
		judgeCounterTxt = new FlxText(20, 0, 0, "", 20);
		judgeCounterTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		judgeCounterTxt.borderSize = 1.25;
		judgeCounterTxt.scrollFactor.set();
		judgeCounterTxt.visible = !ClientPrefs.data.hideHud;
		if(ClientPrefs.data.judgeCounter) updateJudgement();
		judgeCounterTxt.screenCenter(Y);
		if(ClientPrefs.data.judgeCounter) uiGroup.add(judgeCounterTxt);
	...
	public dynamic function updateJudgement()
	{
		...
		judgeCounterTxt.text = 'Sick: $sicks\nGood: $goods\nBad: $bads\nOh Boy: $shits\nMiss: $misses\n\nMax: $maxCombo'; //oh boy reference kms
	}
	...
	function startSong():Void
	{
		...
		if(hasMeta && metadata.creditDelay != null)
		{
			new FlxTimer().start(metadata.creditDelay / playbackRate, function(tmr:FlxTimer)
			{
				// credit popup
				songPopup.init();
			});
	...
	override public function update(elapsed:Float)
	{
		...
		else if (!paused && updateTime)
		{
			...
			if(ClientPrefs.data.timeBarType == 'Time Left')
				...
				timeTxt.text = FlxStringUtil.formatTime(secondsTotal, false) + ' / ' + FlxStringUtil.formatTime(songLength / 1000);
		}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = placement + (43 * (daLoop - (xoff / 2))) - 90 + ClientPrefs.data.comboOffset[2];
...
typedef SongMeta = //throwing metadata stuff down here
{
	var title:String;
	var artist:String;
	var creditDelay:Null<Float>;
}